...
const AddRoom = () => {
  ...
  const {axios, getToken}=useAppContext()
    ...
    const onSubmitHandler = async(e)=>{
      e.preventDefault()
      //Check if all imputs are filled
    ...
    <form onSubmit={onSubmitHandler}>
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
          ...
          <select value={input.roomType} onChange={e=>setInputs({...input,roomType:e.target.value})}
        ...
        <div>
          ...
          mt-1 rounded p-2 w-24' value={input.pricePerNight} onChange={e=>setInputs({...input,pricePerNight:e.target.value})}/>
        </div>
      </div>
